...
function hapusKamar()
{
    ...
    // Periksa apakah ID Kamar valid sebelum menghapus
    if (!empty($id_kamar)) {
        $query = mysqli_query($koneksi, "DELETE FROM kamar WHERE id_kamar=$id_kamar");

        if ($query) {
            // Jangan cetak pesan di sini
...
if (isset($_POST['submit'])) {
    tambahKamar();
} else if (isset($_GET['hapus_kamar'])) { // Menggunakan $_GET karena aksi hapus dilakukan melalui URL
    hapusKamar();
